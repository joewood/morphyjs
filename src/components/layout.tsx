...
export interface ICssGridLayout {
    width: number;
    height: number;
...
export interface IChildGridAssignment {
    style: ISizePosition;
    columnStart: number;
    columnEnd?: number;
    rowStart: number;
...
function getMaxRowColCells(layout: ICssGridLayout, children: IChildGridAssignment[]) {
    ...
    const placedChildrenCol = children.filter(
        c => (!c.columnEnd || c.columnStart === c.columnEnd + 1) 
    );
    const placedChildrenRow = children.filter(
        c =>  (!c.rowEnd || c.rowStart === c.rowEnd + 1)
    );
    const placedChildrenByColumn = groupBy(placedChildrenCol, p => p.columnStart);
    ...
    const placedChildrenByRow = groupBy(placedChildrenRow, p => p.rowStart);
...
function getRowColPositions(layout: ICssGridLayout, children: IChildGridAssignment[]) {
...
export function getSizePositions(layout: ICssGridLayout, children: Dictionary<IChildGridAssignment>): Dictionary<ISizePosition> {
